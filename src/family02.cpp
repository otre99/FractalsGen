...
void Family02::Init(const FractalParameters &p) {
    ...
    default: funct_ = std::bind(Family02::FastPowNInline, std::placeholders::_1, std::placeholders::_2, n_);
    }
}

float Family02::CalcFinalNormMandelbrot(const cmplx &c) const
{
    int iter=1;
    dbltype x=c_.real();
    dbltype y=c_.imag();
...
float Family02::CalcFinalNormJulia(const cmplx &z) const {
    int iter=0;
    dbltype x=z.real();
    dbltype y=z.imag();
...
//////////////////////////////////////////
float Family02::CalcEscapeJulia(const cmplx &z) const {
    int iter=0;
    dbltype x = z.real();
    dbltype y = z.imag();
...
float Family02::CalcEscapeMandelbrot(const cmplx &c) const {
    int iter=1;
    dbltype x = c_.real();
    dbltype y = c_.imag();
    for (;;){
