...
float Family03::CalcFinalNormMandelbrot(const cmplx &c) const
{
    return CalcFinalNormJulia(c);
}


float Family03::CalcFinalNormJulia(const cmplx &z) const {
    int iter=1;
    dbltype rdem;
    dbltype x1, y1, x2, y2;
    dbltype x=z.real();
    ...
    for (; iter<max_iter_;++iter) {
        x1 = x; y1 = y;
        funct_(x,y); x*=n_; y*=n_; rdem = x*x+y*y;
...
//////////////////////////////////////////
float Family03::CalcEscapeJulia(const cmplx &z) const {
    int iter=1;
    dbltype rdem;
    dbltype x1, y1, x2, y2;
    dbltype x=z.real();
...
float Family03::CalcEscapeMandelbrot(const cmplx &c) const {
