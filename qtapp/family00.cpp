...
{
    //qDebug() << " Family00::CalcFinalNormMandelbrot() "<<  double(c.real()) << double(c.imag());
    int iter=1;
    dbltype x, y, dem;
...
double Family00::CalcFinalNormJulia(const cmplx &z) const {
    //qDebug() << " Family00::CalcFinalNormJulia() "<<  double(z.real()) << double(z.imag());
    int iter=0;
    dbltype x, y, dem;
    dbltype xx, yy, tmp1, tmp2, tmp0, q2;
    x = z.real();
...
double Family00::CalcEscapeJulia(const cmplx &z) const {
    //qDebug() << " Family00::CalcEscapeJulia() "<<  double(z.real()) << double(z.imag());
    int iter=0;
    dbltype x, y, dem;
    dbltype xx, yy, tmp1, tmp2, tmp0, q2;
...
double Family00::CalcEscapeMandelbrot(const cmplx &c) const {
    //qDebug() << " Family00::CalcEscapeMandelbrot() "<<  double(c.real()) << double(c.imag());
    int iter=0;
    dbltype x, y, dem;
    dbltype xx, yy, tmp1, tmp2, tmp0, q2;
